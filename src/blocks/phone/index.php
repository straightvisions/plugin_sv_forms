...
class phone extends sv_gutenform {
	...
	public function render_block( array $attr ): string {
		...
		require( $this->get_path( 'lib/frontend/tpl/phone.php' ) );
	...
	public function load_block_assets() {
		if ( ! is_admin() ) {
			$this->get_parent()->get_script( 'phone' )->set_is_enqueued();
		}
	}

	private function register_block() {
		register_block_type(
			'straightvisions/sv-gutenform-phone', array(
				...
				'attributes'		=> array(
					...
					'label' => array(
						'type' => 'string',
						'default' => __( 'Phone Label', 'sv_posts' ),
					),
					'name' => array(
						'type' > 'string',
					),
	...
	protected function get_input_attr(): string {
		return $this->get_root()->sv_gutenform->get_default_input_attr( $this->block_attr, 'tel' );
	}
}